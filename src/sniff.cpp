...
static const int maxkeylen = 6;
static const int reclen = maxkeylen + 1;
static const int hashbytes = sizeof(hash_t) * reclen;
...
static const char *fmtbytes(unsigned long bytes, char *buf, int n)
{
	bzero(buf, n);

	if (bytes > 1024 * 2)
		sprintf(buf, "%lu KB", bytes / 1024L);
	else
...
static const char *fmtrate(unsigned long bits, char *buf, int n)
{
	bzero(buf, n);

	if (bits >= 3 * 1024)
		sprintf(buf, "%lu Kb/s", bits / 1024L);
	else
		sprintf(buf, "%lu b/s", bits);
...
{
	return hash + index * reclen;
}

static int hashget(int index, int offset)
{
...
static int compare(const void *p1, const void *p2)
{
	...
	int val = h2[maxkeylen] - h1[maxkeylen];
...
static const hash_t *fillkey(hash_t *key, hash_t k0, hash_t k1, hash_t k2, hash_t k3, hash_t k4)
{
	key[0] = k0;
	key[1] = k1;
	key[2] = k2;
...
static void hashnode(const node *n)
{
	...
	hash_t key[maxkeylen];

	addhash(fillkey(key, proto, saddr, daddr, sport, dport), n->len);
}
...
static void showhash(time_t seconds)
{
	...
	for (unsigned int i = 0; i < thash; i++) {
		...
		if (v) {
			...
			if ((len = namelen(hashget(i, 1))) > namepad)
				namepad = len;
			if ((len = namelen(hashget(i, 2))) > namepad)
				namepad = len;
		}
	}
	...
	if (total) {
		...
		for (int internal = 0; internal < 2; internal++) {
			...
			for (unsigned int i = 0; i < thash; i++) {
				hash_t val = hashget(i, maxkeylen);

				if (val && internal == isInternal(hashget(i, 1))) {
					if (count == 0) {
						print(4, "idx");
						if (doproto)
							print(6, "proto");
					...
					if (isInternal((addr_t)hashget(i, 1)))
						sent += val;
					else
						recv += val;
					...
					if (count++ < showlines) {
						...
						if (doports) {
							printf(" %5u", hashget(i, 3));
							printf(" %5u", hashget(i, 4));
						}

						printf(" %6u", hashget(i, maxkeylen + 1));
						printf(" %10s", fmtbytes(val, buf, sizeof(buf)));
						printf(" %10s", fmtrate(val * 8L / seconds, buf, sizeof(buf)));
						printf(" %6.2f%%", pct);

						if (dohosts) {
							addr_t saddr = hashget(i, 1);
							addr_t daddr = hashget(i, 2);
